...
class AppLog {
  ...
  AppLog.error(this.message, [this.stackTrace]) {
    if (kDebugMode) {
      log(
        '\x1B[31m[${AppStrings.name}] - $message\x1B[0m',
  ...
  AppLog.success(this.message, [this.stackTrace]) {
    if (kDebugMode) {
      log(
        '\x1B[32m[${AppStrings.name}] - $message\x1B[0m',
        stackTrace: stackTrace,
        name: 'Success',
        level: 800,
      );
  ...
  AppLog.info(this.message, [this.stackTrace]) {
    if (kDebugMode) {
      log(
        '\x1B[33m[${AppStrings.name}] - $message\x1B[0m',
        stackTrace: stackTrace,
        name: 'Info',
        level: 900,
      );
  ...
  AppLog(this.message, [this.stackTrace]) {
    if (kDebugMode) {
      log(
        '\x1B[37m[${AppStrings.name}] - $message\x1B[0m',
